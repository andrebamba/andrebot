...
if (msg.startsWith(prefix + 'USERINFO')) {
  ...
  if (msg === prefix + 'DAILY') {
    if (userData[sender.id + message.guild.id].lastDaily != moment().format('L')) {
      ...
      message.channel.send({embed:{
        title:"Daily Reward",
        description:"You got $500 bang bucks added to your account!"
      }})
    } else {
      message.channel.send({embed:{
        title:"Daily Reward",
        description:"You already collected your daily amount! Get your next dose in " + moment().endOf('day').fromNow()
      }})
    }
// Runs with bot.on
bot.on('ready', () => {
